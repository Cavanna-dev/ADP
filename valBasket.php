...
<?php setNewCommand($db, $_SESSION['customer']['idUser'], $_SESSION['panier']['total'], $_SESSION['panier']['price_total']); ?>
<?php $id_command = $db->lastInsertId(); ?>
...
<?php foreach ($_SESSION['panier']['article'] as $id_basket_product): ?>
    ...
    <?php while ($r_basket_product = $r_basket_products->fetch(PDO::FETCH_OBJ)) { ?>
        ...
        <?php
        $array_sellers[$r_basket_product->avId] = "Vendeur : " . $r_basket_product->custName . " "
                . $r_basket_product->custFirstName . " - " . $email_to_buy . ". Produit : " . $r_basket_product->artName . " - €"
                . $r_basket_product->price . "<br />";
        ?>
        <?php setAvailableOnNull($db, $r_basket_product->avId, $id_command); ?>
    <?php } ?> 
<?php endforeach; ?>
